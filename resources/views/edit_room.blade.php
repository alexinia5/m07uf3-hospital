...
<body class="p-0 m-0 bg-blue-50">
    ...
    max-sm:p-4 max-sm:flex-col max-sm:gap-5">
      <x-navbar.navbar></x-navbar.navbar>
        <main class="flex items-center justify-center w-full p-6 rounded-xl border border-neutral-200 bg-[url('backblue.jpeg')] bg-cover bg-center h-screen">
          @if ($errors->any())
            <div class="bg-red-100 px-6 py-4 mx-2 my-4 rounded-md text-lg flex flex-col items-start mx-auto max-w-lg absolute z-10 left-52">
              ...
              <span class="text-red-800 font-semibold pl-6">Cannot update</span>
          ...
          <div class="bg-white w-full max-w-lg border-2 border-zinc-200 rounded-xl p-4 flex flex-col gap-6 items-center">
            <h1 class="text-2xl font-semibold">Update room</h1>
            <x-forms.update-form :data="['Availability', 'Date Admission', 'Floor number', 'Patient']"/>
          </div>
        </main>
    </div>
</body>
